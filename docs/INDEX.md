<!-- D-R1-STATUS: ACTIVE LAW -->

# Document index

Generated file; do not hand-maintain.

Generation command: `python3 tools/generate_docs_index.py`

Total Markdown documents: 322
Total text records: 94

Groups reflect only the first-line D-R1 machine header, never legacy body `Status:` text.
UNCLASSIFIED records remain visible reconciliation debt; no authority is inferred here.
Text records are listed for corpus visibility only. Presence here does not infer status, authority, classification, adoption, or currentness.

## ACTIVE LAW (79)

- `AGENTS.md`
- `CURRENT_STATE.md`
- `README.md`
- `docs/DESIGN_CONTINUITY.md`
- `docs/DESIGN_CONTINUITY_PHASE_RECONCILIATION_2026_09_04.md`
- `docs/HANDOFF_CANDIDATE_06_STAGES_01_05_RECURSIVE_AUDIT_2026_09_07.md`
- `docs/INDEX.md`
- `docs/KYMAEAN_CROSS_PRODUCT_VISUAL_WEBSITE_LAUNCH_ROADMAP_01.md`
- `docs/KYMAEAN_ENGINEERING_INFORMED_VISUAL_TRANSLATION_PROTOCOL_01.md`
- `docs/KYMAEAN_EXPERIENCE_ONTOLOGY_SYNTHESIS_01.md`
- `docs/KYMAEAN_HERO_INTEGRATION_HARNESS_01.md`
- `docs/KYMAEAN_HERO_METHOD_02_NEUTRAL_GEOMETRY_SCAFFOLD_FORMAT_01.md`
- `docs/KYMAEAN_HERO_SUCCESSOR_DESIGN_CONSTRUCTION_METHOD_02.md`
- `docs/KYMAEAN_HERO_SURFACE_ESCAPE_GATE_01.md`
- `docs/KYMAEAN_HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_01.md`
- `docs/KYMAEAN_HERO_SURFACE_OWNERSHIP_OPERATIONAL_DEFINITION_01.md`
- `docs/KYMAEAN_LANE_A_O0_REINTEGRATION_01.md`
- `docs/KYMAEAN_LANE_A_STAGE_CARRIER_SELECTION_01.md`
- `docs/KYMAEAN_LANE_A_STAGE_CONE_INTERACTION_01.md`
- `docs/KYMAEAN_LANE_B_IDENTITY_SOURCE_READINESS_01.md`
- `docs/KYMAEAN_LANE_B_WEBSITE_V0_HERO_ARTWORK_BRIEF_02.md`
- `docs/KYMAEAN_STAGE_CORE_BASELINE_AND_BACKGROUND_SUCCESSOR_AUTHORITY_01.md`
- `docs/KYMAEAN_VISUAL_DESIGN_WORKFLOW_AUTHORITY_01.md`
- `docs/KYMAEAN_WEBSITE_V2_2_RESPONSIVE_VISUAL_STANDARD_01.md`
- `docs/PRIOR_CONVERGENCE_EVIDENCE.md`
- `docs/PROJECT_REASONING_TASK_SCOPE_OPTIMIZATION_PROTOCOL.md`
- `docs/RENDERER_CARTOGRAPHY_PHASE_RECONCILIATION_2026_09_06.md`
- `docs/RENDERER_ISOLATION_PROTOCOL.md`
- `docs/VISUAL_ARCHIVE_PROTOCOL.md`
- `docs/evidence/BELLWEATHER_ALTERNATE_VISUAL_FREEZE_01.md`
- `docs/evidence/BELLWEATHER_FOUNDATIONAL_FRONT_FACING_MASCOT_01.md`
- `docs/evidence/BELLWEATHER_IMAGE5_MORNING_VARIATION_FREEZE_01.md`
- `docs/evidence/BELLWEATHER_MEDIATE_VISUAL_FREEZE_01.md`
- `docs/evidence/DESIGN_LEDGER.md`
- `docs/evidence/HERO_CANDIDATE_06_METHOD_02_D9_MECHANISM_GEOMETRY_INVESTIGATION_01_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_INTEGRATION_HARNESS_01_NARROW_CORRECTION_COMPLETION.md`
- `docs/evidence/HERO_METHOD_02_VISUAL_MECHANISM_DIRECTION_RECONSIDERATION_03_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_SUCCESSOR_DESIGN_CONSTRUCTION_METHOD_02_DIRECTOR_APPROVAL.md`
- `docs/evidence/LANE_B_INTEGRATED_DEPLOYMENT_LAWS_APPROVAL_01.md`
- `docs/evidence/LANE_B_MINERAL_THEATER_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/LANE_B_O3_WORDMARK_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/LANE_B_QUIET_STAGE_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/LANE_B_QUIET_STAGE_EXTENSION_LAWS_APPROVAL_01.md`
- `docs/evidence/LANE_B_STAGE_INSPIRED_VISUAL_PHYSICS_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/LANE_B_WEBSITE_V0_SITE_SPINE_APPROVAL_01.md`
- `docs/evidence/ODR_33_HERO_CAUSAL_OBSERVABILITY_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/SYM_SYMBOL_EXPLORATION_FRAMEWORK_01.md`
- `docs/evidence/WEBSITE_BELLWEATHER_PRODUCTION_ROLLBACK_01.md`
- `docs/evidence/WEBSITE_BELLWEATHER_PRODUCTION_ROLLBACK_LIVE_VERIFICATION_01.md`
- `docs/evidence/WEBSITE_HOMEPAGE_CONCEALED_PRODUCTION_EXACT_STAGE_TRANSLATION_03.md`
- `docs/evidence/WEBSITE_HOMEPAGE_CONCEALED_PRODUCTION_MOTION_CHOREOGRAPHY_01.md`
- `docs/evidence/WEBSITE_HOMEPAGE_CONCEALED_PRODUCTION_REFINEMENT_02.md`
- `docs/evidence/WEBSITE_HOMEPAGE_CONCEALED_PRODUCTION_SYNTHESIS_01.md`
- `docs/evidence/WEBSITE_HOMEPAGE_CONCEPT_RESET_CONCEALED_PRODUCTION_DOORWAY_01.md`
- `docs/evidence/WEBSITE_HOMEPAGE_VISION_RECOVERY_AND_STAGE_REFINEMENT_01.md`
- `docs/evidence/WEBSITE_PLACEHOLDER_FOUNDATIONAL_STAGE_ARCH_LIGHT_SYNTHESIS_01.md`
- `docs/evidence/WEBSITE_PLACEHOLDER_FOUNDATION_REFINEMENT_REENTRY_01.md`
- `docs/evidence/WEBSITE_PLACEHOLDER_V2_1_ATMOSPHERIC_FINISH_01.md`
- `docs/evidence/WEBSITE_PLACEHOLDER_V2_SECRECY_FIRST_DIRECTION_01.md`
- `docs/evidence/WEBSITE_REFERENCE17_DOORWAY_ONLY_RECONCILIATION_01.md`
- `docs/evidence/WEBSITE_SOL_FRESH_CHAT_HANDOFF_CONCEALED_PRODUCTION_2026_09_17.md`
- `docs/evidence/WEBSITE_V2_2_APPARENT_SIZE_PARITY_HOSTED_AUDIT_01.md`
- `docs/evidence/WEBSITE_V2_2_APPARENT_SIZE_PARITY_LOCAL_AUDIT_01.md`
- `docs/evidence/WEBSITE_V2_2_APPARENT_SIZE_PARITY_PRODUCTION_CLOSEOUT_01.md`
- `docs/evidence/WEBSITE_V2_2_APPARENT_SIZE_PARITY_STUDY_01.md`
- `docs/evidence/WEBSITE_V2_2_MASCOT_INTEGRATION_CANDIDATE_HOSTED_AUDIT_01.md`
- `docs/evidence/WEBSITE_V2_2_MASCOT_INTEGRATION_CANDIDATE_LOCAL_AUDIT_01.md`
- `docs/evidence/WEBSITE_V2_2_MASCOT_INTEGRATION_EXPLORATION_01.md`
- `docs/evidence/WEBSITE_V2_2_MASCOT_INTEGRATION_PRODUCTION_VERIFICATION_01.md`
- `docs/evidence/WEBSITE_V2_2_MASCOT_PLACEMENT_STUDY_01.md`
- `docs/evidence/WEBSITE_V2_2_TEMPORAL_INTERACTION_ACCESSIBILITY_STUDY_01.md`
- `docs/evidence/WEBSITE_V2_2_VIEWPORT_CENTERED_LETTERING_COMPARISON_01.md`
- `docs/evidence/WEBSITE_V2_2_VIEWPORT_CENTERED_SUCCESSOR_HOSTED_AUDIT_01.md`
- `docs/evidence/WEBSITE_V2_2_VIEWPORT_CENTERED_SUCCESSOR_LOCAL_AUDIT_01.md`
- `docs/evidence/WEBSITE_V2_2_VIEWPORT_CENTERED_SUCCESSOR_PRODUCTION_CLOSEOUT_01.md`
- `docs/templates/HERO_METHOD_02_CONSTRUCTION_RECORD_TEMPLATE.md`
- `docs/templates/HERO_METHOD_02_CONSTRUCTION_REVIEW_PACKAGE_TEMPLATE.md`
- `docs/templates/HERO_METHOD_02_RESPONSIVE_SEMANTIC_ENVELOPE_TEMPLATE.md`
- `docs/templates/HERO_METHOD_02_TWO_PERSON_NECESSITY_MATRIX_TEMPLATE.md`

## SUPERSEDED (9)

- `docs/HANDOFF_R0_CROSS_SURFACE_ROBUSTNESS_VALIDATION.md`
- `docs/HANDOFF_R0_RENDERER_CARTOGRAPHY.md`
- `docs/HANDOFF_R0_RENDERER_ISOLATION_CROSS_BRANCH_SYNTHESIS.md`
- `docs/HANDOFF_R0_VISUAL_REFOUNDATION_CLEAN_SLATE_REDISCOVERY.md`
- `docs/HANDOFF_R1_CROSSING_LANGUAGE.md`
- `docs/KYMAEAN_ENTRY_EXPERIENCE_DECISION_PROPOSAL_01.md`
- `docs/KYMAEAN_LANE_B_WEBSITE_V0_HERO_ARTWORK_BRIEF_01.md`
- `docs/KYMAEAN_PRODUCT_EXPERIENCE_MODEL_DECISION_PROPOSAL_01.md`
- `docs/RENDERER_CARTOGRAPHY_PROTOCOL.md`

## HISTORICAL EVIDENCE (234)

- `docs/BELLWEATHER_MASCOT_DESIGN_FRESH_CHAT_HANDOFF_2026_09_16.md`
- `docs/HANDOFF_R0_TO_APP_WEBSITE_DESIGN_SYNTHESIS_01.md`
- `docs/HANDOFF_V0_4_PRESENCE.md`
- `docs/HANDOFF_V0_5B_SURFACE_BLIND_DIVERGENCE.md`
- `docs/HANDOFF_V0_5_FRESH_PALETTE_DIVERGENCE.md`
- `docs/HANDOFF_V0_HERO_SURFACE_FALSIFICATION_CRITERIA_REVIEW_2026_09_06.md`
- `docs/HANDOFF_VID_01_AFTER_V0R10.md`
- `docs/KYMAEAN_AAS_01_AUTHORITATIVE_ANSWER_SOURCING_PROPOSAL.md`
- `docs/KYMAEAN_AAS_V1_SOURCE_SUFFICIENCY_MATRIX_RESULT.md`
- `docs/KYMAEAN_APP_ARCHITECTURE_TO_VISUAL_REQUIREMENTS_CROSSWALK_01.md`
- `docs/KYMAEAN_APP_ARCHITECTURE_TO_VISUAL_REQUIREMENTS_PATCH_0014_ADDENDUM_01.md`
- `docs/KYMAEAN_APP_ARCHITECTURE_TO_VISUAL_REQUIREMENTS_PATCH_0015_ADDENDUM_01.md`
- `docs/KYMAEAN_APP_INFORMATION_RELATIONSHIP_ARCHITECTURE_PROPOSAL_01.md`
- `docs/KYMAEAN_APP_SHELL_NAVIGATION_GRAMMAR_PROPOSAL_01.md`
- `docs/KYMAEAN_APP_TASK_FIRST_LOW_FIDELITY_INTERACTION_PROTOTYPE_01.md`
- `docs/KYMAEAN_APP_WEBSITE_DESIGN_SYNTHESIS_BLUEPRINT_01.md`
- `docs/KYMAEAN_ARS_01_ANSWER_ROUTE_SUFFICIENCY_SIGNALING_PROPOSAL.md`
- `docs/KYMAEAN_ARS_R1_ANSWER_STATE_SURFACE_INTEGRATION_RESULT.md`
- `docs/KYMAEAN_ARS_V1_ANSWER_STATE_LANGUAGE_MATRIX_RESULT.md`
- `docs/KYMAEAN_ATTRIBUTABLE_AFTERMATH_A1_RECONCILIATION_01.md`
- `docs/KYMAEAN_ATTRIBUTABLE_AFTERMATH_A1_RESULT_01.md`
- `docs/KYMAEAN_ATTRIBUTABLE_AFTERMATH_EXPERIMENT_A1_RENDERER_PACKET.md`
- `docs/KYMAEAN_ATTRIBUTABLE_AFTERMATH_GATE_01.md`
- `docs/KYMAEAN_ATTRIBUTABLE_AFTERMATH_PROMPT_AUTHORIZATION_01.md`
- `docs/KYMAEAN_BCA_01_BOUNDED_CONTEXT_ANSWER_PROPOSAL.md`
- `docs/KYMAEAN_BCA_V1_ONE_QUESTION_ONE_ANSWER_RESULT.md`
- `docs/KYMAEAN_CDT_01_CONTEXT_DISCLOSURE_TRANSITION_PROPOSAL.md`
- `docs/KYMAEAN_CDT_V1_BOUNDED_CONTEXT_INVOCATION_RESULT.md`
- `docs/KYMAEAN_CHARACTER_BOUNDED_WATCH_PERFORM_CONTRACT_01.md`
- `docs/KYMAEAN_CHARACTER_REPRESENTATION_REQUIREMENTS_PROPOSAL_01.md`
- `docs/KYMAEAN_CIR_01_CHARACTER_IDENTITY_RECOVERABILITY_PROPOSAL.md`
- `docs/KYMAEAN_CIR_R1_FIRST_USE_IDENTITY_DISCOVERABILITY_RESULT.md`
- `docs/KYMAEAN_CIR_T1R1_ADAPTIVE_IDENTITY_EXPLICITNESS_VISUAL_SEQUENCE_RESULT.md`
- `docs/KYMAEAN_CIR_T1V1_ADAPTIVE_IDENTITY_EXPLICITNESS_STATE_MATRIX_RESULT.md`
- `docs/KYMAEAN_CIR_T1_ADAPTIVE_IDENTITY_EXPLICITNESS_PROPOSAL.md`
- `docs/KYMAEAN_CIR_V1_CHARACTER_IDENTITY_RECOVERABILITY_MATRIX_RESULT.md`
- `docs/KYMAEAN_CONTINUOUS_WORKSPACE_GRAYSCALE_WIREFRAME_01.md`
- `docs/KYMAEAN_CONTINUOUS_WORKSPACE_WIREFRAME_PROPOSAL_01.md`
- `docs/KYMAEAN_CRV_01_CHARACTER_REPRESENTATION_VIABILITY_RESULT.md`
- `docs/KYMAEAN_CSI_01_CONTINUOUS_STAGE_INTEGRATION_PROPOSAL.md`
- `docs/KYMAEAN_CSI_R1_CONTINUOUS_STAGE_INTEGRATION_RESULT.md`
- `docs/KYMAEAN_G1_CONTROLLED_HERO_INTEGRATION_TEST_01.md`
- `docs/KYMAEAN_HERO_CANDIDATE_02_COMPARATIVE_SYNTHESIS.md`
- `docs/KYMAEAN_HERO_CANDIDATE_02_RENDERER_EXECUTION_README.md`
- `docs/KYMAEAN_HERO_CANDIDATE_02_RENDERER_PACKET_G1.md`
- `docs/KYMAEAN_HERO_CANDIDATE_02_RENDERER_PACKET_G2.md`
- `docs/KYMAEAN_HERO_CANDIDATE_02_RENDERER_PACKET_G3.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_CONSTRUCTION_STAGES_01_05.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_ROUND_04_FAILURE_SYNTHESIS_01.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_STAGE_06_NEUTRAL_GEOMETRY_SCAFFOLD.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_STAGE_06_NEUTRAL_GEOMETRY_SCAFFOLD_AUDIT.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_STAGE_06_NEUTRAL_GEOMETRY_SCAFFOLD_CONSTRUCTION_RECORD.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_STAGE_07_STERILE_ARTWORK_TRANSLATION_PACKAGE.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_TRANSFER_FIDELITY_ANALYSIS_01.md`
- `docs/KYMAEAN_HERO_CANDIDATE_04_METHOD_02_TRANSFER_PACKAGE_DELTA_02.md`
- `docs/KYMAEAN_HERO_CANDIDATE_05_METHOD_02_CONSTRUCTION_STAGES_01_05.md`
- `docs/KYMAEAN_HERO_CANDIDATE_05_METHOD_02_D1_D4_MECHANISM_GEOMETRY_INVESTIGATION_01.md`
- `docs/KYMAEAN_HERO_CANDIDATE_06_METHOD_02_CONSTRUCTION_STAGES_01_05.md`
- `docs/KYMAEAN_HERO_CANDIDATE_06_METHOD_02_D9_MECHANISM_GEOMETRY_INVESTIGATION_01.md`
- `docs/KYMAEAN_HERO_CONSTRAINT_RECONCILIATION_01.md`
- `docs/KYMAEAN_HERO_EVIDENCE_RECLASSIFICATION_AFTER_ODR_33_01.md`
- `docs/KYMAEAN_HERO_MECHANISM_CLASS_INVESTIGATION_01.md`
- `docs/KYMAEAN_HERO_METHOD_02_POST_ROUND_05_CAUSAL_TOPOLOGY_REVIEW_01.md`
- `docs/KYMAEAN_HERO_METHOD_02_VISUAL_MECHANISM_DIRECTION_INVESTIGATION_02.md`
- `docs/KYMAEAN_HERO_METHOD_02_VISUAL_MECHANISM_DIRECTION_RECONSIDERATION_03.md`
- `docs/KYMAEAN_HERO_PRE_RENDER_REVIEW_PACKAGE_01.md`
- `docs/KYMAEAN_HERO_PRE_RENDER_REVIEW_PACKAGE_02.md`
- `docs/KYMAEAN_HERO_PRE_RENDER_REVIEW_PACKAGE_03.md`
- `docs/KYMAEAN_HERO_QUALIFYING_ROUND_01_CANDIDATE_SPECIFICATION.md`
- `docs/KYMAEAN_HERO_QUALIFYING_ROUND_02_CANDIDATE_SPECIFICATION.md`
- `docs/KYMAEAN_HERO_QUALIFYING_ROUND_03_CANDIDATE_SPECIFICATION.md`
- `docs/KYMAEAN_HERO_RENDERER_METHOD_01.md`
- `docs/KYMAEAN_HERO_STERILE_RENDERER_PACKET_01.md`
- `docs/KYMAEAN_HERO_STERILE_RENDERER_PACKET_02.md`
- `docs/KYMAEAN_HERO_STERILE_RENDERER_PACKET_03.md`
- `docs/KYMAEAN_HERO_THREE_ROUND_EXPERIMENTAL_EXHAUSTION_REVIEW_01.md`
- `docs/KYMAEAN_LANE_A_ARCHIVE_CAUSALITY_01.md`
- `docs/KYMAEAN_LANE_A_CHARACTER_RECASTING_01.md`
- `docs/KYMAEAN_LANE_A_DEGRADATION_FAILURE_STATES_01.md`
- `docs/KYMAEAN_LANE_A_INTEGRATION_CLOSURE_01.md`
- `docs/KYMAEAN_LANE_A_LONG_SCENE_01.md`
- `docs/KYMAEAN_LANE_A_O0_STATE_COVERAGE_01.md`
- `docs/KYMAEAN_LANE_A_PRODUCTION_LIBRARY_01.md`
- `docs/KYMAEAN_LANE_A_TAKE_ACCEPTANCE_01.md`
- `docs/KYMAEAN_LANE_B_BESPOKE_WORDMARK_CONSTRUCTION_02.md`
- `docs/KYMAEAN_LANE_B_HERO_CANDIDATE_01_EVALUATION.md`
- `docs/KYMAEAN_LANE_B_INTEGRATED_BRAND_SURFACE_TEST_01.md`
- `docs/KYMAEAN_LANE_B_QUIET_STAGE_EXTENSION_TEST_01.md`
- `docs/KYMAEAN_LANE_B_STAGE_INSPIRED_VISUAL_PHYSICS_SYNTHESIS_01.md`
- `docs/KYMAEAN_LANE_B_THRESHOLD_K_CONTEXT_DEPLOYMENT_01.md`
- `docs/KYMAEAN_LANE_B_THRESHOLD_K_CONTROLLED_REFINEMENT_01.md`
- `docs/KYMAEAN_LANE_B_THRESHOLD_K_WORDMARK_INTEGRATION_01.md`
- `docs/KYMAEAN_LANE_B_WEBSITE_V0_MATERIAL_ROLES_01.md`
- `docs/KYMAEAN_LANE_B_WEBSITE_V0_SITE_SPINE_01.md`
- `docs/KYMAEAN_LANE_B_WEBSITE_VISUAL_WORLD_COMPOSITION_01.md`
- `docs/KYMAEAN_LANE_B_WORDMARK_DIVERGENCE_01.md`
- `docs/KYMAEAN_ODR_33_HERO_CAUSAL_OBSERVABILITY_EVIDENCE_AUDIT_01.md`
- `docs/KYMAEAN_R1_FALSIFICATION_DIRECTOR_APPROVAL_01.md`
- `docs/KYMAEAN_RESIDUAL_PRESENCE_REPLICATION_GATE_01.md`
- `docs/KYMAEAN_RESIDUAL_PRESENCE_REPLICATION_R1_EVALUATION_01.md`
- `docs/KYMAEAN_RESIDUAL_PRESENCE_REPLICATION_R1_SYNTHESIS.md`
- `docs/KYMAEAN_RESIDUAL_PRESENCE_REPLICATION_RENDERER_PACKET_01.md`
- `docs/KYMAEAN_SIPD_01_STAGE_INFORMATION_PRESENCE_PROGRESSIVE_DISCLOSURE_PROPOSAL.md`
- `docs/KYMAEAN_SIPD_V1_LIVE_STAGE_PRESENCE_RESULT.md`
- `docs/KYMAEAN_SPR_01_STAGE_PRESENCE_REDUCTION_PROPOSAL.md`
- `docs/KYMAEAN_SPR_R1_STAGE_PRESENCE_REDUCTION_RESULT.md`
- `docs/KYMAEAN_TASK_FIRST_LOW_FIDELITY_STATE_WIREFRAME_PROPOSAL_01.md`
- `docs/KYMAEAN_TCV_01_TEMPORAL_CHARACTER_CONTINUITY_RESULT.md`
- `docs/KYMAEAN_VID_01_AFI_01_ACTION_FIELD_INTERDEPENDENCE_PROPOSAL.md`
- `docs/KYMAEAN_VID_01_ARI_01_ACTION_RESPONSE_INTERLOCK_PROPOSAL.md`
- `docs/KYMAEAN_VID_01_BAF_01_BOUNDED_ACCESS_FIELD_PROPOSAL.md`
- `docs/KYMAEAN_VID_01_BAF_CF1_CROSS_FORMAL_ROBUSTNESS_PROPOSAL.md`
- `docs/KYMAEAN_VID_01_DFI_01_DEPICTION_FIELD_INDEPENDENCE_PROPOSAL.md`
- `docs/KYMAEAN_VID_01_DIRECTOR_HYPOTHESIS_MAP.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_01_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_02_LATERAL_PACKET.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_02_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_03_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_04_LATERAL_PACKET.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_04_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_05_CONTROLLED_FORMAL_LATERAL_PACKET.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_05_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_06_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_07_COMPARATIVE_ACTION_FIELD_PACKET.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_07_COMPARATIVE_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_08_ACTION_RESPONSE_INTERLOCK_PACKET.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_08_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_09_BOUNDED_ACCESS_FIELD_PACKET.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_09_RESULT.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_10_BOUNDED_ACCESS_CROSS_FORMAL_PACKET.md`
- `docs/KYMAEAN_VID_01_V0_RENDER_10_RESULT.md`
- `docs/KYMAEAN_VID_01_VISUAL_IDENTITY_DIVERGENCE_ENTRY.md`
- `docs/KYMAEAN_VISUAL_ASSET_MATERIALIZATION_ROADMAP_01.md`
- `docs/KYMAEAN_VISUAL_SYSTEM_GRAMMAR_PROPOSAL_01.md`
- `docs/KYMAEAN_VSG_A_B_CONVERGENCE_READINESS_AUDIT_01.md`
- `docs/KYMAEAN_VSG_A_B_EVIDENCE_COMPARISON_01.md`
- `docs/KYMAEAN_VSG_CONTROLLED_MULTISTATE_STUDIES_01.md`
- `docs/KYMAEAN_VSG_ISOLATED_POSITIVE_RENDER_PACKETS_01.md`
- `docs/LANE_B_G1_CONTROLLED_INTEGRATION_APPROVAL_CONDITIONS_01.md`
- `docs/R0_CROSS_LAB_SYNTHESIS_ROUND_1.md`
- `docs/R0_CROSS_MEDIUM_SEMANTIC_ALLOCATION_AUDIT_01.md`
- `docs/R0_RENDERER_CARTOGRAPHY_CFM_01A_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_CFM_01B_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_CFM_01C_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_CFM_01_SYNTHESIS.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PAC_01A_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PAC_01B_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PAC_01C_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PAC_01D_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PAC_01E_RESULT.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PAC_SYNTHESIS_01.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PERSISTENT_AFFORDANCE_CARRIER_01.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PERSISTENT_AFFORDANCE_CARRIER_02.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PERSISTENT_AFFORDANCE_CARRIER_03.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PERSISTENT_AFFORDANCE_CARRIER_04.md`
- `docs/R0_RENDERER_CARTOGRAPHY_PERSISTENT_AFFORDANCE_CARRIER_05.md`
- `docs/R0_RENDERER_CARTOGRAPHY_RETROSPECTIVE_01.md`
- `docs/R0_RENDERER_CARTOGRAPHY_SEMANTIC_OBSERVABILITY_01.md`
- `docs/R0_RENDERER_CARTOGRAPHY_SEMANTIC_OBSERVABILITY_SYNTHESIS_02.md`
- `docs/R0_TEMPORAL_OBSERVABILITY_EXPERIMENT_01.md`
- `docs/R0_TEMPORAL_OBSERVABILITY_EXPERIMENT_02.md`
- `docs/R0_TEMPORAL_OBSERVABILITY_TEO_01_RESULT.md`
- `docs/R0_TEMPORAL_OBSERVABILITY_TEO_02_RESULT.md`
- `docs/R0_VISUAL_RESEARCH_FINAL_SYNTHESIS_01.md`
- `docs/RENDERER_ISOLATION_CHECKPOINT_RI_AL.md`
- `docs/evidence/BELLWEATHER_FORMAL_GOWN_TURNAROUND_01.md`
- `docs/evidence/BELLWEATHER_MOTION_AND_MASCOT_VISION_STUDIES_01.md`
- `docs/evidence/BELLWEATHER_VISION_SYNTHESIS_AND_NATURAL_EXPRESSION_01.md`
- `docs/evidence/HERO_ARTWORK_BRIEF_01_ARCHIVE_INDEX.md`
- `docs/evidence/HERO_ARTWORK_BRIEF_01_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_ARTWORK_BRIEF_01_SOURCE_SYNTHESIS.md`
- `docs/evidence/HERO_CANDIDATE_02_EVALUATION.md`
- `docs/evidence/HERO_CANDIDATE_03_MECHANISM_DIRECTION_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_CONSTRUCTION_REVIEW_PACKAGE_01.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_CONSTRUCTION_STAGES_01_05_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_CONTINUATION_SAFE_PROXY_02_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_ROUND_04_EVALUATION_01.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_ROUND_04_EXECUTION_READINESS_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_ROUND_05_EVALUATION_01.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_ROUND_05_EXECUTION_READINESS_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_STAGE_06_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_STAGE_07_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_STRUCTURAL_MASS_TRANSFER_PROXY_01_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_04_METHOD_02_TRANSFER_PACKAGE_DELTA_02_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_05_METHOD_02_CONSTRUCTION_STAGES_01_05_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_05_METHOD_02_D1_D4_MECHANISM_GEOMETRY_INVESTIGATION_01_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_CANDIDATE_05_METHOD_02_STAGE_06_PREFLIGHT_EVALUATION_01.md`
- `docs/evidence/HERO_CONSTRAINT_RECONCILIATION_01_DIRECTOR_DECISION.md`
- `docs/evidence/HERO_CONSTRAINT_RECONCILIATION_01_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_INTEGRATION_HARNESS_01_RENDERING_DEFECT_01.md`
- `docs/evidence/HERO_MECHANISM_CLASS_INVESTIGATION_01_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_METHOD_02_DIRECTION_02_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_METHOD_02_MECHANISM_NEUTRAL_TOOLING_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_METHOD_02_VISUAL_MECHANISM_DIRECTION_INVESTIGATION_02_DIRECTOR_AUTHORIZATION.md`
- `docs/evidence/HERO_METHOD_02_VISUAL_MECHANISM_DIRECTION_INVESTIGATION_02_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_PRE_RENDER_REVIEW_PACKAGE_01_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_PRE_RENDER_REVIEW_PACKAGE_01_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_PRE_RENDER_REVIEW_PACKAGE_02_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_PRE_RENDER_REVIEW_PACKAGE_02_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_PRE_RENDER_REVIEW_PACKAGE_03_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_PRE_RENDER_REVIEW_PACKAGE_03_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_01_EVALUATION_01.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_01_HARNESS_COMPLETION_01.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_01_PROVENANCE_AND_HARNESS_CORRECTION_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_02_EVALUATION_01.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_02_EXECUTION_INTEGRITY_DEFECT_01.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_02_PROVENANCE_AND_CLOSURE_01.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_02_REPLACEMENT_EXECUTION_DIRECTOR_AUTHORIZATION.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_03_CANDIDATE_SPECIFICATION_DIRECTOR_APPROVAL.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_03_CANDIDATE_SPECIFICATION_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_03_EVALUATION_01.md`
- `docs/evidence/HERO_QUALIFYING_ROUND_03_PROVENANCE_AND_CLOSURE_01.md`
- `docs/evidence/HERO_RENDERER_METHOD_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/HERO_RENDERER_METHOD_DIRECTOR_AUTHORIZATION_01.md`
- `docs/evidence/HERO_RENDERER_METHOD_RECURSIVE_AUDIT_01.md`
- `docs/evidence/HERO_RENDERER_METHOD_STATUS_RECONCILIATION_01.md`
- `docs/evidence/HERO_SUCCESSOR_DESIGN_CONSTRUCTION_METHOD_02_RECURSIVE_AUDIT.md`
- `docs/evidence/HERO_SURFACE_METHOD_BOUNDARY_CORRECTION_2026_09_06.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_DEFINITION_FIRST_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_AUTHORIZATION_CORRECTION_01.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_HANDOFF_READINESS_01.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_RECURSIVE_AUDIT_01.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_RECURSIVE_AUDIT_PASS_02.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_REVIEW_PACKET_01.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_FALSIFICATION_CRITERIA_SCOPE_CLOSE_01.md`
- `docs/evidence/HERO_SURFACE_OWNERSHIP_OPERATIONAL_DEFINITION_DIRECTOR_APPROVAL_01.md`
- `docs/evidence/HERO_THREE_ROUND_EXPERIMENTAL_EXHAUSTION_REVIEW_01_DIRECTOR_DECISION.md`
- `docs/evidence/HERO_THREE_ROUND_EXPERIMENTAL_EXHAUSTION_REVIEW_01_RECURSIVE_AUDIT.md`
- `docs/evidence/R0_TO_DESIGN_SYNTHESIS_TRANSITION_APPROVAL.md`
- `docs/evidence/WEBSITE_SOL_FRESH_CHAT_HANDOFF_REFERENCE17_PLACEHOLDER_SUCCESSOR_2026_09_19.md`
- `docs/renderer/KYMAEAN_HERO_CANDIDATE_01_RENDERER_PACKET_H-A.md`
- `docs/renderer/KYMAEAN_HERO_CANDIDATE_01_RENDERER_PACKET_H-B.md`
- `docs/renderer/KYMAEAN_HERO_CANDIDATE_01_RENDERER_PACKET_H-C.md`

## UNCLASSIFIED (0)


## TEXT RECORDS — INVENTORY ONLY (94)

- `docs/evidence/D_R1_ASTRA_MECHANICAL_CENSUS_ARTIFACT_MANIFEST_01.txt`
- `docs/evidence/D_R1_ASTRA_MECHANICAL_CENSUS_TASK_01.txt`
- `docs/evidence/D_R1_ASTRA_PHASE_2_MECHANICAL_IMPLEMENTATION_TASK_01.txt`
- `docs/evidence/D_R1_ASTRA_PHASE_2_MECHANICAL_IMPLEMENTATION_TASK_02.txt`
- `docs/evidence/D_R1_PHASE_0_CLOSURE_01.txt`
- `docs/evidence/D_R1_PHASE_0_CLOSURE_02.txt`
- `docs/evidence/D_R1_PHASE_2_ADOPTION_01.txt`
- `docs/evidence/D_R1_PHASE_3_BRANCH_DISPOSITION_VERIFICATION_01.txt`
- `docs/evidence/D_R1_PHASE_3_CLOSURE_01.txt`
- `docs/evidence/D_R1_PHASE_4_PUBLICATION_SOURCE_BOUNDARY_01.txt`
- `docs/evidence/D_R1_PHASE_5_CLOSURE_01.txt`
- `docs/evidence/D_R1_PHASE_5_CLOSURE_AUDIT_CONTRACT_01.txt`
- `docs/evidence/D_R1_POST_PHASE_4_NEXT_PHASE_AUTHORITY_GAP_01.txt`
- `docs/evidence/HANDOFF_DESIGN_SOL_PHASE_B_PACKET_ACCUMULATION_2026_09_10.txt`
- `docs/evidence/HERO_CANDIDATE_06_METHOD_02_CONSTRUCTION_STAGES_01_05_RECURSIVE_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_06_METHOD_02_ROUND_06_EVALUATION_01.txt`
- `docs/evidence/HERO_CANDIDATE_06_METHOD_02_STAGE_06_NEUTRAL_GEOMETRY_SCAFFOLD_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_06_METHOD_02_STAGE_07_RECURSIVE_PRE_RENDER_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_06_METHOD_02_STAGE_07_STERILE_ARTWORK_TRANSLATION_PACKAGE_01.txt`
- `docs/evidence/HERO_CANDIDATE_07_METHOD_02_CONSTRUCTION_STAGES_01_05.txt`
- `docs/evidence/HERO_CANDIDATE_07_METHOD_02_CONSTRUCTION_STAGES_01_05_RECURSIVE_AUDIT.txt`
- `docs/evidence/HERO_CANDIDATE_07_METHOD_02_ROUND_07_EVALUATION_01.txt`
- `docs/evidence/HERO_CANDIDATE_07_METHOD_02_STAGE_06_NEUTRAL_GEOMETRY_SCAFFOLD_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_07_METHOD_02_STAGE_07_RECURSIVE_PRE_RENDER_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_07_METHOD_02_STAGE_07_STERILE_ARTWORK_TRANSLATION_PACKAGE_01.txt`
- `docs/evidence/HERO_CANDIDATE_07_METHOD_02_STAGE_6M_NEUTRAL_PHYSICAL_MAQUETTE_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_07_STAGE_6M_PREFLIGHT_TOOLING_DEFECT_01.txt`
- `docs/evidence/HERO_CANDIDATE_08_METHOD_02_ROUND_08_EVALUATION_01.txt`
- `docs/evidence/HERO_CANDIDATE_08_METHOD_02_ROUND_08_FRESH_CHAT_HANDOFF_01.txt`
- `docs/evidence/HERO_CANDIDATE_08_METHOD_02_STAGE_06_EDGE_CLEARANCE_METADATA_CORRECTION_01.txt`
- `docs/evidence/HERO_CANDIDATE_08_METHOD_02_STAGE_06_NEUTRAL_GEOMETRY_SCAFFOLD_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_08_METHOD_02_STAGE_07_RECURSIVE_PRE_RENDER_AUDIT_01.txt`
- `docs/evidence/HERO_CANDIDATE_08_METHOD_02_STAGE_07_STERILE_ARTWORK_TRANSLATION_PACKAGE_01.txt`
- `docs/evidence/HERO_CANDIDATE_08_METHOD_02_STAGE_6M_NEUTRAL_PHYSICAL_MAQUETTE_AUDIT_01.txt`
- `docs/evidence/HERO_DPSC_C01_CONSTRUCTION_D0_D5_01.txt`
- `docs/evidence/HERO_DPSC_C01_LATERAL_CONTINUUM_CORRECTION_D0_D5_01.txt`
- `docs/evidence/HERO_DPSC_C01_NEUTRAL_SCAFFOLD_FALSIFICATION_01.txt`
- `docs/evidence/HERO_DPSC_C01_REVISION_2_NEUTRAL_SCAFFOLD_FALSIFICATION_AND_PROGRAM_CLOSURE_01.txt`
- `docs/evidence/HERO_DPSC_RENDERED_SUCCESSOR_PROGRAM_01.txt`
- `docs/evidence/HERO_HTCR_01_A1_CARRIER_DIVERGENCE_RESULT_01.txt`
- `docs/evidence/HERO_HTCR_01_A2_ADVERSARIAL_FALSIFICATION_RESULT_01.txt`
- `docs/evidence/HERO_HTCR_01_RECURSIVE_CLOSURE_AUDIT_01.txt`
- `docs/evidence/HERO_METHOD_02_D9_S2_CONSTRUCTION_STAGES_01_05_CORRECTED_01.txt`
- `docs/evidence/HERO_METHOD_02_D9_S2_CONSTRUCTION_STAGES_01_05_RECURSIVE_AUDIT_01.txt`
- `docs/evidence/HERO_METHOD_02_D9_S2_CONSTRUCTION_STAGES_01_05_WORKING_01.txt`
- `docs/evidence/HERO_METHOD_02_D9_SUCCESSOR_CANDIDATE_MECHANISM_GEOMETRY_INVESTIGATION_01.txt`
- `docs/evidence/HERO_METHOD_02_D9_SUCCESSOR_CANDIDATE_MECHANISM_GEOMETRY_INVESTIGATION_01_RECURSIVE_AUDIT.txt`
- `docs/evidence/HERO_METHOD_02_D9_SUCCESSOR_CHANGE_TO_POSTURE_MECHANISM_GEOMETRY_INVESTIGATION_02.txt`
- `docs/evidence/HERO_METHOD_02_D9_THREE_RENDER_BUDGET_DIRECTOR_RECONCILIATION_01.txt`
- `docs/evidence/HERO_METHOD_02_NEUTRAL_PHYSICAL_MAQUETTE_TRANSFER_ADDENDUM_01.txt`
- `docs/evidence/HERO_METHOD_02_NEUTRAL_PHYSICAL_MAQUETTE_TRANSFER_ADDENDUM_01_RECURSIVE_AUDIT.txt`
- `docs/evidence/HERO_METHOD_02_POST_ROUND_06_TRANSFER_FIDELITY_REVIEW_01.txt`
- `docs/evidence/HERO_METHOD_02_POST_ROUND_07_TRANSFER_FIDELITY_CAUSAL_TOPOLOGY_REVIEW_01.txt`
- `docs/evidence/HERO_METHOD_02_POST_ROUND_08_CONSTRAINT_RECONCILIATION_RECOMMENDATION_01.txt`
- `docs/evidence/HERO_METHOD_02_STAGE_6M_PHYSICAL_UNION_RENDERER_CORRECTION_01.txt`
- `docs/evidence/HERO_METHOD_02_STAGE_6M_TOOLING_CLOSURE_AND_D9_REENTRY_01.txt`
- `docs/evidence/HERO_METHOD_02_STAGE_6M_TOOLING_FORMAT_IMPLEMENTATION_01.txt`
- `docs/evidence/HERO_METHOD_02_STAGE_6M_TOOLING_HARDENING_01.txt`
- `docs/evidence/HERO_METHOD_02_TRANSFER_SURFACE_SUFFICIENCY_INVESTIGATION_01.txt`
- `docs/evidence/HERO_POST_PROGRAM_RECONCILIATION_01.txt`
- `docs/evidence/HERO_TRANSLATION_CARRIER_RECONCILIATION_PROGRAM_01.txt`
- `docs/evidence/PHASE_B_REENTRY_CROSS_PROJECT_RECONCILIATION_01.txt`
- `docs/evidence/SHARED_BRAND_ORTHOGONAL_REENTRY_AUDIT_02.txt`
- `docs/evidence/SHARED_BRAND_ORTHOGONAL_REENTRY_AUDIT_03.txt`
- `docs/evidence/SHARED_BRAND_VISUAL_IDENTITY_DIVERGENCE_REENTRY_AUDIT_01.txt`
- `docs/evidence/STAGE_CONE_INTERACTION_01_ORACLE.txt`
- `docs/evidence/renderer/C04-ACLT-01_ROUND04_RENDERER_HANDOFF.txt`
- `docs/evidence/renderer/C04-ACLT-01_ROUND05_RENDERER_HANDOFF.txt`
- `docs/evidence/renderer/C04-ACLT-01_STAGE07_STERILE_RENDERER_PACKET.txt`
- `docs/evidence/renderer/C04-ACLT-01_TRANSFER_FIDELITY_RENDERER_PACKET_02.txt`
- `docs/evidence/renderer/C06-ASPB-01_STAGE07_STERILE_RENDERER_PACKET.txt`
- `docs/evidence/renderer/C07-CBSR-01_STAGE07_SANITIZED_MAQUETTE_REFERENCE_SHA256.txt`
- `docs/evidence/renderer/C07-CBSR-01_STAGE07_STERILE_RENDERER_PACKET.txt`
- `docs/evidence/renderer/C08-EWTF-01_STAGE07_SANITIZED_MAQUETTE_REFERENCE_SHA256.txt`
- `docs/evidence/renderer/C08-EWTF-01_STAGE07_STERILE_RENDERER_PACKET.txt`
- `docs/evidence/renderer/CTF_01_R1_T1_PRIMARY_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/CTF_01_R1_T3_TRANSFER_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/CTF_01_R2_T2_PRIMARY_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/CTF_01_R2_T4_TRANSFER_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/CTF_01_R3_T1_TRANSFER_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/CTF_01_R3_T3_PRIMARY_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P1_PRIMARY_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P1_TRANSFER_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P2_PRIMARY_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P2_TRANSFER_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P3_PRIMARY_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P3_TRANSFER_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P4_PRIMARY_STERILE_PACKET_01.txt`
- `docs/evidence/renderer/VFD_01_P4_TRANSFER_STERILE_PACKET_01.txt`
- `intelligence/README.txt`
- `prototypes/website-concealed-production-motion-choreography-01/motion-review-notes.txt`
- `site/README.txt`
- `updates/KYMAEAN_PLACEHOLDER_INITIAL_PRODUCTION_DEPLOYMENT_2026_09_14.txt`
- `updates/README.txt`
