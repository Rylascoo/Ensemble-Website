...
1. resolve the current authoritative checkpoint;
2. identify the actual decision, defect, artifact, experiment, or outcome requested;
3. include all directly dependent source reading and synthesis needed for that outcome;
...
5. include targeted contradiction, edge-case, and falsification analysis;
...
Combine work because the pieces depend on one another, not because a larger reply appears more capable.
...
Do not ask the Director to choose or restate information when:

- authoritative GitHub/Drive/project evidence already answers it;
- a previously approved phase or proposal already supplies the needed authority;
- the decision is an internal method choice that does not change product/creative authority;
...
- major creative convergence or identity selection;
- a frozen architecture redesign;
- implementation authorization when only design/architecture was approved;
- a new product-scope boundary;
...
- no worthwhile improvement that belongs inside the current objective;
