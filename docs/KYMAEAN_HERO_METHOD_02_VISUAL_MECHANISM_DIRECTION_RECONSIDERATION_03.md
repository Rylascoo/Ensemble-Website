...
Reopen the Method 02 direction-family decision after:

- Candidate 04 failed twice at the same causal-topology layer under materially different transfer representations; and
- Candidate 05's stronger D1+D4 geometry failed the new transfer-visible pre-existing-form ambiguity test before renderer execution.

The frozen hero contract remains unchanged.

## Evidence that must now be respected

### What is proven achievable

Across Rounds 02–05:

- two specific people can remain visually primary;
- Human Necessity can pass;
- Changed-condition legibility can pass in some geometry families;
- Altered Possibility / Removal Test can pass under gravity-dependent posture geometry;
- Epistemic Restraint, Surface Escape and restrained surface ownership are repeatedly achievable;
- responsive clustering and natural identity coexistence can be designed prospectively.

### What repeatedly fails

The specific D1 proposition that **A's passive body load must be visibly inferred as the cause of a field deformation that creates B's position** has now failed at three increasingly controlled levels:

1. line-reference renderer -> terrain naturalization;
2. filled-mass renderer -> sculpted-basin naturalization;
3. body-conforming D1+D4 transfer preflight -> still coherent as pre-shaped support; stronger foldback -> device/platform dominance.

The hidden burden is not required by Hero Brief 02. ODR-33 already resolved that exact prior/current causal authorship need not be proven in the hero.

Therefore continuing to require A to author the changed field is no longer the smallest route to the frozen hero burden.

## D1 disposition

**ASYMMETRIC COUPLED LOAD-TRANSFER HUMAN RELATION is RETIRED AS THE ACTIVE NEXT-CANDIDATE DIRECTION.**

This does not erase its evidence. It successfully taught how to preserve Human Necessity, S1 and responsive human clustering while exposing the transfer-visible causal-author ambiguity.

## Reconsidered families

### D2 — reciprocal counterweight equilibrium

Still offers pair irreducibility, but depends even more heavily than D1 on hidden force inference and invites seesaw/hammock/rig/device structure.

**Disposition: REJECT for next candidate.**

### D3 — changed-field-enabled body-to-body support

Still offers the strongest human-first potential because the people, not the field, carry the final support event.

Its original weaknesses remain: dance, rescue, acrobatics, intimacy and performance shorthand.

**Disposition: RETAIN, but only with a deliberately non-performative asymmetric support role.**

### D8 — residual trace + present physical consequence

Still useful as an evidence-design principle because it separates proof that the shared condition is changed from proof of what present human possibility that change creates.

**Disposition: ELEVATE AS A SUPPORTING PRINCIPLE, not an autonomous direction.**

## New synthesis D9 — asymmetric altered-posture passive human brace
...
This reduces performance/dance/rescue shorthand while preserving two-person necessity.
...
- passive brace may still read as helping/rescue;
- shoulder/back contact may read as staged dance/performance;
...
## Comparative reconsideration

Design-analysis estimates only.

| Direction | Pair irreducibility | Change recoverability | Physical necessity | Human-first potential | Hidden-force burden | Anti-device robustness | Anti-intimacy/performance robustness | Mobile viability |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| retired D1 current-load transfer | 5 | 4 | 5 | 5 | **2** | 3 | 4 | 4 |
| D2 reciprocal counterweight | 5 | 3 | 5 | 3 | 2 | 2 | 3 | 2 |
| generic D3 mutual body support | 5 | 3 | 4 | 5 | 4 | 4 | 2 | 4 |
| **D9 asymmetric passive human brace + residual change** | **5** | **5** | **5** | **5** | **5** | **4** | **3** | **5** |

## Recommended new direction

# ASYMMETRIC ALTERED-POSTURE PASSIVE HUMAN BRACE

Candidate construction should no longer ask a renderer or viewer to infer who caused the field change.
...
If recursively audited clean, this direction authorizes only a new candidate construction investigation. It authorizes no scaffold, renderer packet, reference transfer or image generation.
