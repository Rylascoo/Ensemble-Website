...
The secrecy-first direction and preflight/review/promotion evidence are recorded in `docs/evidence/WEBSITE_PLACEHOLDER_V2_SECRECY_FIRST_DIRECTION_01.md`; Design Ledger entry L-152 records live closure. Public copy remains limited to `KYMÆAN`, `Something is taking shape.`, and `In development`. The implementation uses exact recovered-control Threshold K geometry and inherited O3 namework geometry; its atmospheric field is deterministic HTML/CSS and carries no product disclosure or final-brand authority.
