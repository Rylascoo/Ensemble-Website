<!-- D-R1-STATUS: ACTIVE LAW -->
# Ensemble Website Current State

Updated: 2026-09-15

## Authority
`Rylascoo/Ensemble-Website` is Design/UI/website authority. `Rylascoo/Ensemble-Project` is read-only Product/Engineering input. Recover live refs and `AGENTS.md` before using this file.

## Active stream
APPUI remains isolated on `design/appui01-working-compositions-2026-09-14`; concurrent Website publication remains separate. Last reconciled Website `main` is `10434eb5eb79793055d002e8742b288fb5268e12`; that stream creates no app UI/palette/state/interaction/shared-brand-final authority.

## Frozen architecture
**Phase 1 application UI architecture/layout is FROZEN PASS and detailed-section coverage is CLOSED CLEAN. Phase 2 static Stage/live-production coverage is CLOSED PASS.** Dynamic/transcript-dependent and Product-gated Stage work remains held/open.

## Product / Engineering boundary
Fresh Engineering `main@bd32532133b4dbc66d2ffd1a7f77c3d0a2fd38dd` remains E0-D active/on hold after both P01 slots were consumed; E0-E remains blocked. No Phase-D Product semantics invalidate the frozen shell/Stage envelope or Phase-3 representative set.

## Phase 3 static app visual system
CLR-01 **F2 — Monochrome + Intruder** is Director-selected for Light. **D3 — Ink Aubergine** is Director-selected as the static APP VISUAL Dark direction after 10/10 representative-section convergence.

Canonical static app-visual authority is now `docs/KYMAEAN_APPUI_PHASE_3_STATIC_APP_VISUAL_SYSTEM_01.md`, supported by `docs/evidence/APPUI_01_PHASE_3_STATIC_APP_VISUAL_SYSTEM_CONSOLIDATION_01.json`.

The system consolidates selected Light F2 + Dark D3 with MAT-01 F1 Subtractive Channel, TYP-02 F1 Ratio Ladder and STA-01 F2 Ring + Edge Witness where architecture requires them. Stage remains invariant dark and outside app-theme translation.

This authority is design-reference scope only: no production tokens, semantic status colors, Stage successor/motion, native WinUI/runtime behavior, shipping assets, website palette or shared-brand-final authority are frozen.

## Next
Create and validate a compact static app-visual reference carrier from the canonical specification, then audit it against the ten accepted section carriers. The reference carrier must remain design evidence rather than a production token package or substitute for application-state authority.
