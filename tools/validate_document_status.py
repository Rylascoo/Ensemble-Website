...
from generate_docs_index import INDEX, ROOT, STATUSES, collect_statuses, render_index
...
def main():
    try:
        ...
        if (ROOT / INDEX).read_bytes() != render_index(statuses):
            raise ValueError(f"{INDEX}: differs from the canonical corpus index; regenerate it")
        print(f"PASS: {len(statuses)} Markdown documents; headers and index valid")
