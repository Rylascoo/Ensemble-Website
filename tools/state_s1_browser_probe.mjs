...
const harness=path.join(root,'prototypes','state-s1','harness.html');
fs.rmSync(profile,{recursive:true,force:true});fs.mkdirSync(profile,{recursive:true});
const args=['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--allow-file-access-from-files','--disable-extensions','--remote-debugging-port=0',`--user-data-dir=${profile}`,'about:blank'];
...
try{
  ...
  for(let i=0;i<150;i++){
    const ev=await send('Runtime.evaluate',{expression:'window.__STATE_S1_DIAGNOSTIC__||null',returnByValue:true});
    last=ev?.result?.value||null;if(last){diag=last;break}await sleep(100);
  }
  if(!diag)throw new Error('STATE-S1 diagnostic missing: '+JSON.stringify(last));
