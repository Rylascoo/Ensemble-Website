...
const harness=path.join(root,'prototypes','cone-s1','harness.html');
...
try{
  ...
  for(let i=0;i<150;i++){
    const ev=await send('Runtime.evaluate',{expression:'window.__CONE_S1_DIAGNOSTIC__||null',returnByValue:true});
    last=ev?.result?.value||null;if(last){diag=last;break}await sleep(100);
  }
  if(!diag)throw new Error('CONE-S1 diagnostic missing: '+JSON.stringify(last));
