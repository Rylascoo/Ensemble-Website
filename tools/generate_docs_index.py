...
def text_paths(root):
    """Inventory text records without parsing or inferring status."""
    paths = []
    for directory, dirs, files in os.walk(root):
        dirs[:] = sorted(d for d in dirs if d != ".git")
        for name in files:
...
def render_index(statuses, texts):
    ...
    lines = [
        ...
        "Text records are inventoried for visibility only; no status, authority or classification is inferred here.", "",
    ]
    for status in STATUSES:
        ...
        lines.append("")
    lines.extend([f"## TEXT RECORDS — UNGOVERNED ({len(texts)})", ""])
    lines.extend(f"- `{path}`" for path in sorted(texts))
    lines.append("")
