...
EXPECTED_STAGE6_CLEARANCE_BASIS = "normalized_4_decimal_stage6_manifest_geometry_at_registered_left_support_root"
SUPERSEDED_STAGE6_PLANNING_STATUS = "SUPERSEDED_UNDEFORMED_BASELINE_PLANNING_PROVENANCE"
...
def main():
    ...
    assert audit["B_support_to_free_edge_planning_clearance_status"] == SUPERSEDED_STAGE6_PLANNING_STATUS
    assert abs(float(audit["B_support_to_free_edge_changed_clearance_px"]) - EXPECTED_STAGE6_CLEARANCE_PX) < 1e-9
    assert audit["B_support_to_free_edge_changed_clearance_basis"] == EXPECTED_STAGE6_CLEARANCE_BASIS
    assert abs(float(audit["B_support_to_free_edge_integer_svg_clearance_px"]) - 102.8) < 1e-9
    ...
    print("C08 normalized Stage 6 support-root edge clearance source px", corrected_clearance_px)
    print("C08 integer SVG support-root edge clearance source px", audit["B_support_to_free_edge_integer_svg_clearance_px"])
