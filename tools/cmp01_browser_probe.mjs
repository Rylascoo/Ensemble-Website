...
const [forcedArg,reducedArg,widthArg,diagPath,profile,screenshotPath]=process.argv.slice(2);
const width=Number(widthArg),height=6500,browser=process.env.BROWSER;
if(!browser) throw new Error('BROWSER not set');
const forced=forcedArg==='forced', reduced=reducedArg==='reduced';
...
try{
 ...
 const nav=await send('Page.navigate',{url:'http://127.0.0.1:8765/prototypes/cmp-01/harness.html'});if(nav?.errorText)throw new Error('navigate: '+nav.errorText);
 for(let i=0;i<150;i++){const ev=await send('Runtime.evaluate',{expression:'!!window.__CMP01_API__',returnByValue:true});if(ev?.result?.value)break;if(i===149)throw new Error('CMP-01 API missing');await sleep(100);}
 const ev=await send('Runtime.evaluate',{expression:'window.__CMP01_API__.diagnostic()',awaitPromise:true,returnByValue:true});
 const diag=ev?.result?.value;if(!diag)throw new Error('CMP-01 diagnostic missing');
 await send('Runtime.evaluate',{expression:'document.activeElement?.blur();document.body.focus?.()',returnByValue:true});
 const keyboardFocus=[];
 for(let i=0;i<14;i++){
