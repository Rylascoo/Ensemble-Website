...
const output = process.argv[2] || path.join(os.tmpdir(), 'website-mascot-integration-candidate.json');
const captureRoot = process.argv[3] || path.join(os.tmpdir(), 'website-mascot-integration-candidate-captures');
const profile = path.join(os.tmpdir(), `website-mascot-integration-edge-${process.pid}`);
...
try {
  ...
  await evalv(`(()=>{const e=document.getElementById('main');e.focus();e.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowDown',bubbles:true,cancelable:true}));return true})()`); await sleep(620);
  results.interaction.keyboardForward = await evalv('window.__KYM_TEMPORAL__.snapshot()');
  await evalv(`(()=>{window.dispatchEvent(new WheelEvent('wheel',{deltaY:80,bubbles:true}));return true})()`); await sleep(260);
  ...
  results.interaction.focusAfterRetarget = await evalv(`(()=>{const e=document.getElementById('main');e.focus();window.__KYM_TEMPORAL__.advance();window.__KYM_TEMPORAL__.reverse();return {activeId:document.activeElement.id,state:window.__KYM_TEMPORAL__.state}})()`);
  ...
  results.reducedMotion.sequence = await evalv(`(()=>{const states=[];for(let n=0;n<7;n++){window.__KYM_TEMPORAL__.advance();states.push(window.__KYM_TEMPORAL__.state)}return {states,snapshot:window.__KYM_TEMPORAL__.snapshot(),animations:document.getAnimations().filter(a=>a.playState==='running').length}})()`);
  ...
  await evalv('window.__KYM_TEMPORAL__.set(3)'); await capture('wide-person');
  await evalv('window.__KYM_TEMPORAL__.set(5)'); await capture('wide-trace');
  await evalv('window.__KYM_TEMPORAL__.set(7)'); await capture('wide-life');

  await send('Emulation.setDeviceMetricsOverride', { width: 390, height: 844, deviceScaleFactor: 1, mobile: false, screenWidth: 390, screenHeight: 844 });
  await send('Page.navigate', { url: `${origin}/` }); await waitReady();
  await evalv('window.__KYM_TEMPORAL__.set(3)'); await capture('mobile-person');
  await evalv('window.__KYM_TEMPORAL__.set(7)'); await capture('mobile-life');
