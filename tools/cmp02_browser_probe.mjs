...
const width=Number(widthArg),height=10000,fontPercent=Number(fontArg),browser=process.env.BROWSER,pageUrl=process.env.CMP02_URL||'http://127.0.0.1:8765/prototypes/cmp-02/harness.html';
...
try{
 ...
 const nav=await send('Page.navigate',{url:pageUrl});if(nav?.errorText)throw new Error('navigate: '+nav.errorText);
 for(let i=0;i<150;i++){const ev=await send('Runtime.evaluate',{expression:'!!window.__CMP02_API__',returnByValue:true});if(ev?.result?.value)break;if(i===149)throw new Error('CMP-02 API missing');await sleep(100);}
 ...
 await sleep(60);
 const ev=await send('Runtime.evaluate',{expression:'window.__CMP02_API__.diagnostic()',returnByValue:true});
 const diag=ev?.result?.value;if(!diag)throw new Error('CMP-02 diagnostic missing');
