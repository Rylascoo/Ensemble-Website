...
MANIFEST=ROOT/"docs/evidence/MAT_01_FROZEN_CANDIDATE_MANIFEST_01.json"
...
def main():
    ...
    if manifest.get("status")!="FROZEN_BEFORE_BROWSER_EXPOSURE": fail("manifest status")
    ...
    low=text.lower()
    prohibited_tags=["<img","<svg","<canvas","<video","<picture","<object","<embed","<iframe"]
    for token in prohibited_tags:
        if token in low: fail("prohibited element "+token)
    prohibited_css=["linear-gradient","radial-gradient","conic-gradient","backdrop-filter","filter:","animation:","animation-name","transition:","box-shadow:"]
    # box-shadow:none is the only permitted declaration and is checked separately.
    for token in prohibited_css:
        if token=="box-shadow:":
            bad=[m.group(0) for m in re.finditer(r"box-shadow\s*:\s*(?!none\b)[^;}]+",low)]
            if bad: fail("non-none box shadow")
        elif token in low:
            fail("prohibited CSS "+token)
    ...
    for token in required:
        if re.sub(r"\s+","",token) not in compact: fail("missing frozen token "+token)

    if "--probe:" not in text or "--scale:" not in text: fail("compact scaling variables absent")
    print("MAT01_STATIC=PASS")
...
if __name__=="__main__":
    try: main()
        ...
        print("MAT01_STATIC_FAIL:",e,file=sys.stderr); sys.exit(1)
