...
const carrierUrl=pathToFileURL(path.resolve('prototypes/appui-01/whole-production-character-management-functional-study-01.html')).href;
...
try{
 ...
 const ev=await send('Runtime.evaluate',{expression:expr,returnByValue:true});const diag=ev?.result?.value;if(!diag)throw new Error('APPUI Character management diagnostic missing');
