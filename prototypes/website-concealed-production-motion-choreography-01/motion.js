...
function travelAt(t){
  if(t<=.34) return .34*smooth(t/.34);
  if(t<=.58) return .34+.18*smooth((t-.34)/.24);
  return .52+.48*smooth((t-.58)/.42);
}
function pose(p){
  progress=clamp(p);
  ...
  const recognition=Math.exp(-Math.pow((mt-.48)/.115,2));
  const bob=-1.25*Math.sin(Math.PI*5.2*travel);
  const lean=.65*Math.sin(Math.PI*travel)-.9*recognition;
  figure.setAttribute('transform','translate('+x.toFixed(3)+' '+(ground+bob).toFixed(3)+') rotate('+lean.toFixed(3)+') scale('+scale.toFixed(4)+')');
  head.setAttribute('transform','rotate('+(-3.2*recognition).toFixed(3)+' -9 -176)');
...
window.__MOTION01__={
  ...
  diagnostic(){
    ...
    return {
      progress,durationMs:DURATION,motionWindow:[MOTION_START,MOTION_END],
